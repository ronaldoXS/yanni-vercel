...
async function buscarPedidoPorRef(ref) {
...
async function atualizarPedido(id, dados) {
  ...
  const url = `https://firestore.googleapis.com/v1/projects/${FIREBASE_PROJECT}/databases/(default)/documents/pedidos/${id}?${mask}&key=${FIREBASE_API_KEY}`;
...
module.exports = async (req, res) => {
  ...
  if (req.method === 'POST' && pathname === '/webhook') {
    try {
      ...
      if (['approved','paid','APPROVED','PAID'].includes(status)) {
        const pedidoId = await buscarPedidoPorRef(orderId);
        if (pedidoId) {
          await atualizarPedido(pedidoId, {status:'pago',statusPagamento:'aprovado',pagoEm:new Date().toISOString(),valorPago:valor,infinitePayOrderId:orderId});
  ...
  if (req.method === 'POST' && pathname === '/suki-painel') {
    try {
      ...
      const d = await r.json();
      const resposta = d.choices?.[0]?.message?.content || 'Suki não conseguiu conectar.';
